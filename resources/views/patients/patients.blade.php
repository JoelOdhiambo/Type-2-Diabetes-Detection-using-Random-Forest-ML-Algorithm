    ...
    <div class="pt-6 md:p-8 text-center md:text-left w-screen h-screen space-y-4">
        ...
        <div class="shadow-lg rounded-lg overflow-hidden container mx-auto">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div><canvas class="p-10" id="chartBar" style="width:100%;height:300px;"></canvas></div>
                <div>Put graph 2</div>
                <div>Put graph 3</div>
            </div>
    ...
    $(window).on('load', function() {
                        ...
                        return `<div style="display:flex; justify-content:start;" ><td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                 <button style="margin-right:8px;"  class=" diagnose-btn flex items-center bg-green-500 text-white px-2 py-1 text-xs font-semibold  uppercase transition-colors duration-200 transform bg-white rounded hover:bg-green-700  focus:bg-gray-400 focus:outline-none button">
                                     ...
                                     <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                         <button type="button" class="delete-btn flex items-center bg-red-600 text-white px-2 py-1 text-xs font-semibold  uppercase transition-colors duration-200 transform bg-red rounded hover:bg-red-700  focus:bg-gray-400 focus:outline-none">
                                             <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
        ...
        $(`#patients-table tbody`).on('click', '.diagnose-btn', async function() {
            ...
            try {
                const patientData = {
                    patient_id: data.id,
                    patient_records: [data.pregnancies, data.glucose, data.bloodpressure, data.skinthickness, data.insulin, data.bmi, data.diabetespedegreefunction, data.age]
                }
                const response = await ajaxRequest('POST', '{{ route("patient.inference") }}', patientData)
                if (response?.result) {
                    if (response.result?.error) {
                        alert(response.result?.error)
                        return
                    }
                    alert(JSON.stringify(response.result.predictions))
                }
            } catch (error) {
                const message = error?.message || error?.responseJSON?.message || "Something went wrong, unable to diagnose patient"
                alert(message)
            } finally {
        ...
        $(`#patients-table tbody`).on('click', '.edit-btn', async function() {
            ...
            //     const response = await ajaxRequest('PUT', `{{ route("patients.update", "") }}/${data.id}`, editData)
            ...
            //     $(this).html(originalBtn)
            // }
            // $('#edit-patient').attr('action', '/patients/'+ data.id);
        ...
        $(`#patients-table tbody`).on('click', '.delete-btn', function() {
            var data = patientsTable.row($(this).parents('tr')).data();
        })
